...
import { deleteSupplier } from "../../../InventoryManagement/Redux/Supplier/SupplierActions";
...
export default function DeleteSuplierModal({ setDeleteModal, supplierId }) {
  const dispatch = useDispatch();
  const [showToast, setShowToast] = useState(false);
  const supplier = useSelector((state) => state.SupplierReducer.data);
  const supplierToDelete = supplier.find((supplier) => supplier._id === supplierId);
  ...
  <div className="relative bg-d-primary-bg-color text-d-primary-text-color w-[90%] max-w-md p-6 rounded-2xl shadow-xl flex flex-col gap-2 min-h-[200px]">
    ...
    <h4 className="text-xl font-medium text-center mt-1">{supplierToDelete && supplierToDelete["Supplier Name"]}</h4>
